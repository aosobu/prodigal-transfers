...
public interface CustomerComplaintRepo extends JpaRepository<CustomerComplaint,Long> {
    ...
    @Query(value = "SELECT cc.complaint_message FROM complaint.customer_complaint cc where cc.cust_id in (select ct.cust_id from complaint.complaint_transaction ct where ct.transaction_id = :transaction_id) limit 1",nativeQuery = true)
    public String getTransComplaintMessageByTid(@Param("transaction_id") String tid);
}
